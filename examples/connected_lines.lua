-- This is a ARTIMAGEN Lua script that generates a connected set of vertical lines
-- by Petr Cizmar @ NIST
...
imsize = 1024 -- Size of a side of the square image
line_width = 30 -- Width of the defined lines
line_pitch = 70 -- Pitch of the defined lines
line_top = 70 -- Height of the beginnigns of the bends
...
while x1 < imsize do
   x = x1
   x1 = x + line_pitch + line_width
   table.insert(curves, aig_new_curve("segment", {{x,line_top}, {x,imsize-line_top}})) -- definition of all curves
   table.insert(curves, aig_new_curve("bezier", {{x,imsize-line_top}, {x,imsize}, {x1,imsize}, {x1,imsize-line_top}}))

   x = x1
   x1 = x + line_pitch - line_width
   table.insert(curves, aig_new_curve("segment", {{x,imsize-line_top}, {x,line_top}}))
   table.insert(curves, aig_new_curve("bezier", {{x,line_top}, {x,line_top/2}, {x1,line_top/2}, {x1,line_top}}))
end

-- END LINE--
   x = x1
   x1 = x + line_width
   table.insert(curves, aig_new_curve("segment", {{x,line_top}, {x1,line_top}}))

-- NEGATIVE X DIRECTION --
while x > 0 do
   x = x1
   x1 = x - line_pitch - line_width
   table.insert(curves, aig_new_curve("bezier", {{x,line_top}, {x,0}, {x1,0}, {x1,line_top}}))
   table.insert(curves, aig_new_curve("segment", {{x1,line_top}, {x1,imsize-line_top}}))

   x = x1
   x1 = x - line_pitch + line_width
   table.insert(curves, aig_new_curve("bezier", {{x,imsize-line_top}, {x,imsize-line_top/2}, {x1,imsize-line_top/2}, {x1,imsize-line_top}}))
   table.insert(curves, aig_new_curve("segment", {{x1,imsize-line_top}, {x1,line_top}}))
end
...
aig_save_image(im, "connected_lines.tiff","Connected Lines, by Petr Cizmar") --saving of the image to a file
